...
describe("renewal-state", () => {
  ...
  beforeEach(() => {
    cwd = mkdtempSync(join(tmpdir(), "renewal-state-test-"));
  });
  ...
  function stateFile(sessionId: string): string {
    return getRenewalStatePath(cwd, sessionId);
  }
   ...
   * created `.pi/renew/` or the session dir for them yet.
   */
  ...
  function oldFile(name: string, content: string): string {
    const path = join(getRenewalStateDir(cwd), name);
    writeRawFile(path, content);
    setMtime(path, OLD_MTIME);
    return path;
  }

  describe("readRenewalState", () => {
    it("round-trips a record with the context byte-for-byte, whitespace and newlines included", () => {
      ...
      const state = readRenewalState(cwd, id);
      ...
      expect(state!.version).toBe(RENEWAL_STATE_VERSION);
    });
    ...
    it("defaults absent toggles to true when reading a valid record", () => {
      ...
      // full RenewalState passed to writeRenewalState can never produce.
      writeRawFile(
        stateFile(id),
        JSON.stringify(
          {
    ...
    it("throws, naming the file and both versions, for an unknown version", () => {
      ...
      expect(() => readRenewalState(cwd, id)).toThrow(path);
      expect(() => readRenewalState(cwd, id)).toThrow("9");
      expect(() => readRenewalState(cwd, id)).toThrow(String(RENEWAL_STATE_VERSION));
    });
    ...
    it("throws, naming the file and both versions, for a missing version", () => {
      ...
      expect(() => readRenewalState(cwd, id)).toThrow(path);
      expect(() => readRenewalState(cwd, id)).toThrow("undefined");
      expect(() => readRenewalState(cwd, id)).toThrow(String(RENEWAL_STATE_VERSION));
    });
    ...
    it("returns null, not a throw, when no record exists", () => {
      ...
      expect(readRenewalState(cwd, id)).toBeNull();
    });
    ...
    it("keeps records of two distinct session ids fully independent", () => {
      ...
      expect(readRenewalState(cwd, b)).toMatchObject({ context: "context B", includeSummary: false, restartCount: 0 });
    });
  });

  describe("writeRenewalState", () => {
    it("leaves no temp file behind after a successful write", () => {
      ...
      const entries = readdirSync(getRenewalStateDir(cwd));
      expect(entries).toEqual([`renewal-${id}.json`]);
    });
  });
  ...
  describe("resolveRenewalState", () => {
    it("returns the session's own record when one exists, with no previous file", () => {
      ...
      const state = resolveRenewalState(cwd, id);
    ...
    it("adopts a predecessor's record given a previousSessionFile, by rename, not copy", () => {
      ...
      writeRenewalState(cwd, predecessorId, {
        version: RENEWAL_STATE_VERSION,
      ...
      const state = resolveRenewalState(cwd, currentId, previousFile);
    ...
    it("finds a predecessor's record through a real two-deep parentSession chain", () => {
      ...
      const state = resolveRenewalState(cwd, child, parentFile);
    ...
    it("returns null when no record can be found", () => {
      ...
      expect(resolveRenewalState(cwd, id)).toBeNull();
    });
    ...
    it("does not overwrite an existing record under the current id when adopting", () => {
      ...
      writeRenewalState(cwd, currentId, {
        version: RENEWAL_STATE_VERSION,
      ...
      expect(readRenewalState(cwd, predecessorId)).toMatchObject({ context: "predecessor" });
    });
    ...
    it("stops a parentSession cycle without throwing or hanging", () => {
      ...
      const state = resolveRenewalState(cwd, a, fileB);
    ...
    it("stops a parentSession chain at the hop bound without throwing or hanging", () => {
      ...
      const state = resolveRenewalState(cwd, "019fe2dd-9999-7571-a595-6f5b930f66d8", prev);
      expect(state).toBeNull();
    });
  });

  describe("reapRenewalStates", () => {
    it("deletes only stale renewal records, sparing the current session, fresh records and foreign files", () => {
      ...
      const strayTmp = oldFile("renewal-old.json.tmp-4242-abcd1234", "partial");

      reapRenewalStates(cwd, currentId);
    ...
    it("does not throw when the state directory cannot be listed", () => {
      // `.pi/renew` exists but is a file, so readdir fails with ENOTDIR.
      // A sweep is housekeeping and must never take the session down.
      writeRawFile(join(cwd, ".pi", "renew"), "not a directory");
      expect(() =>
        reapRenewalStates(cwd, "019fe2e0-0000-7571-a595-6f5b930f66d8")
      ).not.toThrow();
    });
  });

  describe("RENEWAL_STATE_MAX_AGE_MS", () => {
    it("is exactly 14 days in milliseconds", () => {
      expect(RENEWAL_STATE_MAX_AGE_MS).toBe(14 * 24 * 60 * 60 * 1000);
    });
  });
  ...
  describe("claimRestartOrdinal", () => {
    it("returns 1, 2, 3 across three consecutive calls on one record, and the persisted record ends at 3", () => {
      ...
      expect(readRenewalState(cwd, id)!.restartCount).toBe(1);

      const second = claimRestartOrdinal(cwd, id, "reason two");
      expect(second.ordinal).toBe(2);
      expect(readRenewalState(cwd, id)!.restartCount).toBe(2);

      const third = claimRestartOrdinal(cwd, id, "reason three");
      expect(third.ordinal).toBe(3);
      expect(readRenewalState(cwd, id)!.restartCount).toBe(3);
    });
    ...
    it("persists each call's reason as lastReason, and the third (distinct) reason is what survives", () => {
      ...
      expect(readRenewalState(cwd, id)!.lastReason).toBe(
    ...
    it("with no record on disk, returns { ordinal: 1, record: null } and writes nothing", () => {
      ...
      expect(existsSync(getRenewalStatePath(cwd, id))).toBe(false);
    });
