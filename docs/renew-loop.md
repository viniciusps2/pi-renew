...
└── add-auth/
    ...
    ├── handover-add-auth-old-1.md  # the previous turn's handover — one archive per turn
...
written into, renumbered, or adopted as a handover by a later run. They are ignored like the rest of
...
hundred kilobytes is a real outcome of a long OpenSpec run, and it fails twice over — the next session
